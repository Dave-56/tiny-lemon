# Shopify App Store listing — copy to paste

Use this doc to fill the fields you have info for. Replace placeholders where noted.

**Base URL:** `https://tinylemon.xyz`

---

## App category

- **Primary category:** **Store design > Images and media > Image editor** (Apps that help merchants edit and optimize images.)

### App category details (tags)

You must add at least 1 tag per field (or check “Not applicable” if it truly doesn’t apply).

| Field | What to add |
|-------|-------------|
| **Image optimization** | Add tags that fit: e.g. **Image generation**, **Product photography**, **Studio shots**, or from their list **Auto-optimization** (we auto-create optimized product imagery). Pick whatever the form suggests that matches “generating/creating product images.” |
| **Bulk editing** | If the app doesn’t do classic bulk edit (alt text, file names, format conversion), check **Not applicable for this app**. If there’s a tag like “Batch generation” or “Multiple images per product,” you can use that instead. |

---

## App store listing content

### Introduction (max 100 characters)

```
On-model photos and short fashion videos from flat-lays. No shoot needed.
```
*(74 chars.)*

---

### App details (max 500 characters)

```
Stop waiting on photoshoots. Upload a flat-lay from your factory or supplier and get polished on-model product photos in under 60 seconds — front, three-quarter, and back angles. Then generate a short branded fashion video from the same images — ready for PDPs, Reels, or TikTok. Choose a styling direction, build a custom model, and push everything directly to your products. Built for fashion brands that want pro imagery and motion content without a real shoot.
```
*(~475 chars.)*

---

### Features (3–5 items, max 80 chars each)

1. `Flat-lay to front, 3/4, and back on-model shots from a single upload`
2. `Generate short branded fashion videos from your outfit images`
3. `Push images and video directly to your Shopify products — no export`
4. `Custom model builder + 6 brand styling directions for consistency`
5. `High-res output, upscaling, and one-click downloads`

---

### Demo store URL (optional)

```
https://tinylemon.xyz/try
```
*(Free try flow; no Shopify install required to see the demo.)*

---

### Feature media

*You provide: video or image 1600×900px. Video recommended (under 3 min).*

---

### Screenshots

*You provide: 3+ desktop (and optional mobile) screenshots. Crop out browser chrome. No real PII.*

---

### Integrations (optional, max 6)

Leave blank, or add e.g. **Vercel** if you want to mention hosting. Do not list Shopify.

---

## Support

| Field | Value |
|-------|--------|
| **Preferred support channel** | Support email (or Support portal if you add one) |
| **Support email address** | *You fill: e.g. support@tinylemon.com or your preferred address* |
| **Support portal URL** | Optional: e.g. `https://tinylemon.xyz` or a help URL |
| **Support phone** | Optional |

---

## Resources

| Field | Value |
|-------|--------|
| **Privacy policy URL** | `https://tinylemon.xyz/privacy` |
| **Developer website** | `https://tinylemon.xyz` |
| **Pricing (more info)** | `https://tinylemon.xyz/pricing` |
| FAQ / Changelog / Tutorial / Additional docs | Optional; add if you have URLs |

---

## Pricing details

**At least one public plan required.** Use your real plan names and prices from the Partner Dashboard / Billing API. Suggested display text:

| Plan | Display name | Top features (brief) |
|------|----------------|----------------------|
| 1 | **Free** | 1 generation/month during launch, 1 model, full 3-angle set, watermarked. |
| 2 | **Starter** | 30 generations/month, full 3-angle set, high-res, unwatermarked. |
| 3 | **Growth** | 100 generations/month, detail + flat lay, 2 brand styles, priority queue. |
| 4 | **Scale** | 300 generations/month, full set + lifestyle, unlimited models, credit rollover. |

**Optional URL for more pricing info:** `https://tinylemon.xyz/pricing`

---

## App discovery content

### App card subtitle (max 62 characters)

```
Flat-lay to on-model photos and video — no shoot needed.
```
*(61 chars)*

---

### App store search terms (1–5 terms, complete words)

- `product photography`
- `flat lay to studio`
- `fashion product images`
- `on-model photos`
- `studio shots`

---

### Web search content (optional)

- **Title tag (max 60):** `Tiny Lemon – Studio product photos from flat-lays | Shopify app`
- **Meta description (max 160):** `Turn flat-lay product photos into studio shots in 60 seconds. Front, 3/4, and back angles for fashion brands on Shopify. No photographer, no shoot.`

---

## Install requirements

- **Sales channel:** Select **Shopify Online Store** (app is for product imagery on the online store).
- **Geographic requirements:** Leave as “no requirements” unless you restrict by region/currency.

---

## Contact information

- **Merchant review email:** *You fill (where you want review notifications).*
- **App submission email:** *You fill. Add app-submissions@shopify.com and noreply@shopify.com to allowlist.*

---

## App testing information

### Test account

- If the app is install-only (no separate login): select **My app doesn’t require an account to use it** and in **Testing instructions** tell reviewers to install on a development store and use the in-app flow.
- If reviewers need a dev store: *You fill* — e.g. “Install the app on a Shopify development store; use the store’s admin to open the app. No separate username/password.”

### Screencast URL

*You provide: 3–8 min video showing install, onboarding, and core flow (e.g. upload flat-lay → generate → add to product).*

### Testing instructions (example — adjust to match your flow)

```
To test Tiny Lemon:
1. Install the app on your development store (Add app → approve scopes).
2. Open the app from the Shopify admin. You’ll land in the app home.
3. Go to “Dress model” (or main generation screen). Upload a flat-lay product image.
4. Choose angle(s) and start generation. Wait for the result (about 1–2 minutes).
5. Add the generated image to a product from the app, or download.
6. Optional: try “Model builder” and “Brand style” to see custom model and style options.
7. Billing: use “Billing” in the app to see plans; test upgrade flow if needed.
```

---

## Extended marketing copy (for landing pages, ads, blog)

### Headline

**Stop waiting on photoshoots. Start selling faster.**

### Value proposition

You have the garment. You have the flat-lay from your factory or supplier. Tiny Lemon turns that flat-lay into polished, on-model product photos — the kind that actually convert — without booking a photographer, a model, or a studio. Then it goes further: generate a short branded fashion video from those same images, ready for your PDP, Reels, or TikTok Shop.

### How it works

1. Upload a flat-lay photo of your garment
2. Tiny Lemon generates on-model shots in multiple angles — front, three-quarter, and back
3. Add the images to your Shopify products and publish

That's it. Minutes, not weeks.

### Built for fashion brands, not generic AI

This is not a general-purpose image generator. Tiny Lemon is purpose-built for apparel PDP imagery. The AI understands fabric, fit, and garment construction so your product looks like the real thing — not a hallucinated version of it.

### Your brand, your model, your look

Choose from 6 styling directions — Editorial Cool, Minimal Clarity, Street Aesthetic, and more — so every generated photo matches your brand identity. Use the Model Builder to create and save a custom model that appears consistently across your entire catalog. No more mismatched product pages.

### Short fashion videos — no video shoot

Once your outfit images are ready, generate a 5-second branded fashion clip with one click. Motion is shaped by your brand style, so the video feels like yours. Use it on product pages, Instagram Reels, TikTok Shop, or paid creative. More output per SKU, zero extra production.

### Publish to Shopify in one click

Generated images and videos sync directly to your Shopify products as draft listings. No downloading, no manual uploads, no broken workflows. Your catalog stays in one place.

### Plans that scale with you

- **Free** — 1 generation/month during launch, full 3-angle set, watermarked. Try it with zero commitment.
- **Starter ($39/mo)** — 30 generations, full 3-angle sets, high-res, unwatermarked. Perfect for indie brands adding new SKUs.
- **Growth ($99/mo)** — 100 generations, detail close-ups, flat lay output, multiple backgrounds, priority queue. For brands running seasonal drops.
- **Scale ($249/mo)** — 300 generations, lifestyle imagery, unlimited models and brand profiles, credit rollover. For catalogs with 50+ SKUs per season.

### Why merchants switch to Tiny Lemon

- Cut product photo and video costs by 80–90% compared to traditional shoots
- Go from sample to live listing in the same day
- Get photos and short video from one flat-lay — more content per SKU
- Maintain visual consistency across hundreds of SKUs
- No need to coordinate photographers, models, studios, or retouchers
- Works with flat-lays from your factory, supplier, or your own shoot — any background

### Key features reference

| Feature | Description |
|---|---|
| Flat-lay to on-model | Upload a flat-lay, get professional on-model photos with accurate fabric and fit |
| Multi-angle generation | Front hero, three-quarter (45°), and back views from a single upload |
| Brand styling directions | 6 preset styles (Editorial Cool, Minimal Clarity, Street Aesthetic, etc.) to match your brand |
| Custom Model Builder | Generate and save a consistent model for your brand across all products |
| One-click Shopify sync | Push generated images directly to Shopify product listings |
| High-res output | Up to 1K resolution in 2:3 aspect ratio (Starter and above) |
| Image upscaling | Upscale to even higher resolution for hero placements (Growth and above) |
| Short fashion video | Generate a 5-second branded clip from your outfit images — for PDPs, Reels, TikTok |
| Flat-lay validation | Advisory check ensures your upload will produce the best results |
| Batch-friendly | Generate for your entire collection without repeat setup |

---

## Tracking (optional)

- **Google Analytics:** Measurement ID (e.g. G-xxx) and API Secret if you want listing + install tracking.
- **Facebook Pixel / Remarketing:** *You fill if you use them.*

---

*Update this doc when you change pricing, URLs, or positioning.*
